  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (mode === "form") {
        ...
        res = await fetch("http://localhost:4000/api/issue/csv", {
          method: "POST",
          body: formData,
        });
      }
      ...
      if (res.ok) {
        ...
        setMessage({ text: data.error || "Error processing request", type: "error" });
      }
    } catch (error) {
      ...
      <div className="max-w-md mx-auto bg-white rounded-xl shadow-md overflow-hidden md:max-w-2xl">
        ...
        <div className="p-8">
          <div className="flex mb-6 border-b">
            <button
              className={`px-4 py-2 font-medium ${mode === "form" ? "border-b-2 border-indigo-600 text-indigo-600" : "text-gray-500"}`}
              onClick={() => setMode("form")}
            >
              Single Entry
            </button>
            <button
              className={`px-4 py-2 font-medium ${mode === "csv" ? "border-b-2 border-indigo-600 text-indigo-600" : "text-gray-500"}`}
              onClick={() => setMode("csv")}
            >
              Bulk Upload (CSV)
            </button>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <p className="mt-1 text-sm text-gray-500">
                    Upload a CSV file with columns: certificateId, university, studentName, course, date
                  </p>
                </div>
              </div>
            )}
            ...
            <div className="flex items-center justify-between">
              ...
              >
                {isSubmitting ? 'Processing...' : mode === "form" ? 'Issue Certificate' : 'Upload Certificates'}
              </button>
